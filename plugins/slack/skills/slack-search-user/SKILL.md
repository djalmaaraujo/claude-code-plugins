...
- ✅ **Case-insensitive matching** - Matches username, real name, or display name
...
~/.claude/plugins/slack/skills/slack-search-user/search-user.sh "John Smith"

# Capture user ID in a variable
USER_ID=$(~/.claude/plugins/slack/skills/slack-search-user/search-user.sh fulano)
...
3. If not found, searches via Slack API with pagination
4. Automatically caches ALL users from API response (up to 1000 per request)
5. Excludes bots and deleted users
6. Case-insensitive matching across `name`, `real_name`, and `display_name`
7. Returns user ID on stdout for easy integration
...
~/.claude/plugins/slack/skills/slack-search-user/cache-utils.sh count
# Output: Total cached users: 12000

# List all cached users
~/.claude/plugins/slack/skills/slack-search-user/cache-utils.sh list

# Search for users matching a pattern
~/.claude/plugins/slack/skills/slack-search-user/cache-utils.sh search will

# Clear the cache
~/.claude/plugins/slack/skills/slack-search-user/cache-utils.sh clear
```
